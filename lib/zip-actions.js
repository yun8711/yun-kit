...
function zipDist(options) {
  ...
  if (options.build) {
    const buildCmd = options.build === true ? 'npm run build' : `npm run ${options.build}`;
    print.info('开始执行构建项目：' + buildCmd);
    shell.exec(`npm run ${options.build}`);
  ...
  // 获取项目名称
  const name = options.name || require(path.resolve(cwd, 'package.json')).name;
